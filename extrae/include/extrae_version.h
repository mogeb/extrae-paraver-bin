...
/* -=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=- *\
 | @file: $HeadURL: https://svn.bsc.es/repos/ptools/extrae/tags/3.2/3.2.1/src/common/extrae_version.h $
 | @last_commit: $Date: 2015-11-03 10:44:17 +0100 (Tue, 03 Nov 2015) $
 | @version:     $Revision: 3531 $
\* -=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=- */
...
#define EXTRAE_VERSION                     EXTRAE_VERSION_NUMBER(3,2,1)
